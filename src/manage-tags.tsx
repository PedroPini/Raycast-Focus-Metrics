...
import { addTag, getAllTags, loadTags, removeTag, saveTags } from "./sessionStorage";
...
export default function Command() {
  ...
  useEffect(() => {
    ...
    if (!searchText.trim()) {
      ...
      const filtered = tags.filter(
        (tag) =>
          tag.name.toLowerCase().includes(searchText.toLowerCase()) || tag.usageCount.toString().includes(searchText),
      );
  ...
  const updateTagName = async (oldName: string, newName: string): Promise<void> => {
    try {
      // Update stored tags
      const storedTags = await loadTags();
      const updatedStoredTags = storedTags.map((tag: string) => (tag === oldName ? newName : tag));
  ...
  if (showCreateForm) {
    return (
      <Form
        actions={
          <ActionPanel>
            <Action.SubmitForm title="Create Tag" icon={Icon.Plus} onSubmit={handleCreateTag} />
  ...
  if (editingTag) {
    return (
      <Form
        actions={
          <ActionPanel>
            <Action.SubmitForm title="Update Tag" icon={Icon.Checkmark} onSubmit={handleEditTag} />
            <Action title="Cancel" icon={Icon.XMarkCircle} onAction={() => setEditingTag(null)} />
          </ActionPanel>
        }
      >
        <Form.TextField
  ...
  const unusedTagsCount = filteredTags.filter((tag) => tag.usageCount === 0).length;
  const totalUsageCount = filteredTags.reduce((sum, tag) => sum + tag.usageCount, 0);
    ...
    >
          ...
          <List.Item
            ...
            actions={
              <ActionPanel>
                ...
                />
                <Action title="Create New Tag" icon={Icon.Plus} onAction={() => setShowCreateForm(true)} />
                {unusedTagsCount > 0 && (
                  <Action
                    title="Delete Unused Tags"
                    icon={Icon.Trash}
                ...
                <Action title="Refresh" icon={Icon.ArrowClockwise} onAction={loadTagsData} />
              </ActionPanel>
            }
          />
        ))}
      ...
      {filteredTags.length === 0 && !isLoading && (
        <List.EmptyView
          ...
          description={searchText ? `No tags match "${searchText}"` : "Create your first tag to get started"}
          icon={Icon.Tag}
          actions={
            <ActionPanel>
              <Action title="Create New Tag" icon={Icon.Plus} onAction={() => setShowCreateForm(true)} />
