...
export default function Command() {
  ...
  const handleBulkComplete = async (tag: string) => {
    ...
    if (confirmed) {
      try {
        ...
        const runningSessions = tagStats.sessions.filter((s) => s.status === "running");
  ...
  const totalSessions = stats.reduce((sum, tagStats) => sum + tagStats.count, 0);
  const totalDuration = stats.reduce((sum, tagStats) => sum + tagStats.totalDuration, 0);
  const runningSessions = stats.reduce(
    (sum, tagStats) => sum + tagStats.sessions.filter((s) => s.status === "running").length,
    0,
  );
  ...
  return (
      ...
      actions={
        <ActionPanel>
          ...
          <Action
            ...
            onAction={() => setViewMode(viewMode === "summary" ? "detailed" : "summary")}
    ...
    >
        ...
        <List.Section title="Recent Activity">
          {recentSessions.map((session) => (
            <List.Item
              ...
              accessories={[
                {
                  text: session.duration ? formatDuration(session.duration) : "No duration",
                  icon: Icon.Clock,
                },
                {
                  text: session.status,
                  icon: session.status === "running" ? Icon.Play : Icon.Checkmark,
                },
              ]}
              actions={
                <ActionPanel>
                  ...
                  />
                  <Action title="Refresh" icon={Icon.ArrowClockwise} onAction={loadStats} />
                </ActionPanel>
              }
            />
          ))}
      ...
      {/* Tag Statistics */}
      {stats.map((tagStats) => {
        const runningCount = tagStats.sessions.filter((s) => s.status === "running").length;
        ...
        return (
          <List.Section key={tagStats.tag} title={tagStats.tag}>
            <List.Item
              ...
              accessories={[
                {
                  text: runningCount > 0 ? `${runningCount} running` : "All completed",
