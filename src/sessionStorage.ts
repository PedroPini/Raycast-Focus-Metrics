...
export const isFocusRunning = async (): Promise<boolean> => {
  try {
    ...
    const raycastApp = applications.find(
      (app) => app.bundleId?.toLowerCase().includes("raycast") || app.name.toLowerCase().includes("raycast"),
    );
...
export const getSessionStats = async (): Promise<SessionStats[]> => {
  ...
  return Array.from(statsMap.values()).sort((a, b) => b.totalDuration - a.totalDuration);
};
...
export const removeTag = async (tagName: string): Promise<void> => {
  try {
    ...
    const sessions = await loadSessions();
    const updatedSessions = sessions.map((session) => (session.tag === tagName ? { ...session, tag: "" } : session));
