...
import type { Session, SessionArguments, UserPreferences } from "./types";
...
export default function Command() {
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(["social", "messaging", "streaming"]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [tags, preferences] = await Promise.all([getAllTags(), loadUserPreferences()]);
  ...
  const updateRecentTags = async (newTag: string) => {
    const updated = [newTag, ...recentTags.filter((t) => t !== newTag)].slice(0, 5);
  ...
  const handleSubmit = async (values: { goal: string; duration: string; mode: string; tag: string }) => {
    try {
      ...
      const durationText = values.duration ? formatDuration(parseInt(values.duration, 10)) : "No limit";
      await showToast({
  ...
  const handleQuickPreset = (preset: string) => {
    switch (preset) {
      ...
      case "all-distractions":
        setSelectedCategories(["social", "messaging", "streaming", "gaming", "news", "shopping", "entertainment"]);
        break;
      default:
        break;
    }
  ...
  if (showTemplates) {
    return (
      <List
        actions={
          <ActionPanel>
            <Action title="Back to Form" icon={Icon.ArrowLeft} onAction={() => setShowTemplates(false)} />
          </ActionPanel>
        }
      >
        <List.Section title="Quick Start Templates">
          {QUICK_TEMPLATES.map((template) => (
            <List.Item
              ...
              actions={
                <ActionPanel>
                  <Action title="Use Template" icon={Icon.Checkmark} onAction={() => handleTemplateSelect(template)} />
                  <Action title="Back to Form" icon={Icon.ArrowLeft} onAction={() => setShowTemplates(false)} />
                </ActionPanel>
              }
            />
  ...
  if (activeSession) {
    const elapsedTime = Math.floor((Date.now() - activeSession.startTime) / 1000);
    const remainingTime = activeSession.duration ? activeSession.duration - elapsedTime : null;
  ...
  if (showForm) {
    return (
      <Form
        ...
        actions={
          <ActionPanel>
            <Action.SubmitForm title="Start Focus Session" icon={Icon.Play} onSubmit={handleSubmit} />
            ...
            />
            <Action title="Streaming" icon={Icon.Video} onAction={() => handleQuickPreset("streaming")} />
            <Action title="Gaming" icon={Icon.GameController} onAction={() => handleQuickPreset("gaming")} />
            <Action title="All Distractions" icon={Icon.Lock} onAction={() => handleQuickPreset("all-distractions")} />
            <Action
              title="Clear Form"
              icon={Icon.Trash}
      ...
      >
        ...
        <Form.Dropdown id="mode" title="Mode" value={mode} onChange={setMode}>
          <Form.Dropdown.Item value="block" title="Block" icon={{ source: Icon.Lock, tintColor: Color.Red }} />
          <Form.Dropdown.Item value="allow" title="Allow" icon={{ source: Icon.Checkmark, tintColor: Color.Green }} />
        </Form.Dropdown>
          ...
          {recentTags.length > 0 && (
            <Form.Dropdown.Section title="Recent Tags">
              {recentTags.map((tagOption) => (
                <Form.Dropdown.Item key={tagOption} value={tagOption} title={tagOption} icon={Icon.Clock} />
              ))}
          ...
          {availableTags.length > 0 && (
            <Form.Dropdown.Section title="Available Tags">
              {availableTags
                ...
                .map((tagOption) => (
                  <Form.Dropdown.Item key={tagOption} value={tagOption} title={tagOption} />
                ))}
            </Form.Dropdown.Section>
          )}
          {tag?.trim() && !availableTags.includes(tag) && !recentTags.includes(tag) && (
            <Form.Dropdown.Section title="Create New Tag">
              <Form.Dropdown.Item value={tag} title={`Create "${tag}"`} icon={Icon.Plus} />
            </Form.Dropdown.Section>
          )}
        </Form.Dropdown>
